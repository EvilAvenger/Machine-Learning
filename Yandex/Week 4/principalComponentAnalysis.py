...
class  DataLoader(object):
    ...
    def __validate_data(self, url_name):
        result = os.path.exists(url_name)
        return result

    def __get_folder_path(self, url, folder_name):
    ...
    def download_data(self, url_name, folder_name):
        path = self.__get_folder_path(url_name, folder_name)

        if not self.__validate_data(path):
